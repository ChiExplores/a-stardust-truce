...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            error_message = "Your password can't be too similar to your other personal information. Your password must contain at least 8 characters. Your password can't be a commonly used password. Your password can't be entirely numeric."
...
def structure_update_submit(request, data_structures_id):
    ...
    try:
        ...
        ds.properties.set(Property.objects.filter(
            Q(id__in=new.getlist('properties')) & Q(id__in=ds.__get_valid_properties__())))
        ds.save()
...
def structure_methods_submit(request, data_structures_id):
    ...
    try:
        ds.name = new['name']
        ds.description = new['description']
        ds.methods.set(Method.objects.filter(
            Q(id__in=new.getlist('methods')) & Q(id__in=ds.__get_valid_methods__())))
        ds.save()
...
def structure_download_js(request, data_structures_id):
    ...
    response = FileResponse(open(filename, 'rb'),
                            as_attachment=True, filename=f'{ds.name}.js')
    os.remove(filename)
    return response
...
def structure_download_py(request, data_structures_id):
    ...
    response = FileResponse(open(filename, 'rb'),
                            as_attachment=True, filename=f'{ds.name}.py')
    os.remove(filename)
    return response
